...
import AddDialogWorksIn from './AddDialogWorksIn';
...
function createData(station_rif,emp_id) {
  ...
  const rows = data.map(worksIn => createData(worksIn.station_rif,worksIn.emp_id));
  ...
  return (
    <>
      <TableContainer component={Paper} sx={{ marginLeft: 31, marginTop: 10, maxWidth: 'calc(100% - 250px)' }}>
        ...
        {rows.map((row) => (
         <StyledTableRow key={`${row.station_rif}-${row.emp_id}`}>
